...
-- UNIQUE(doctor_id, appointment_date, start_time) yalnızca AYNI başlangıç
-- saatine ikinci kaydı engeller; 10:00-10:30 ile 10:15-10:45 gibi kısmen
-- kesişen aralıkları YAKALAMAZ. MySQL/InnoDB PostgreSQL'deki gibi native
-- bir "EXCLUDE USING gist" (range overlap) constraint sunmadığından,
-- bunu trigger ile uyguluyoruz. CANCELLED randevular çakışma sayılmaz.
--
-- ÖNEMLİ: Trigger tek başına yarış durumunu (race condition) önlemez.
-- İki eşzamanlı transaction aynı anda trigger kontrolünü çalıştırıp
-- ikisi de "boş" görebilir. Bu yüzden uygulama katmanında randevu
-- oluşturma/güncelleme işlemi mutlaka aşağıdaki gibi yapılmalı:
--
--   START TRANSACTION;
...
--     FOR UPDATE;                 -- ilgili satırları kilitler
--   -- sonuç 0 ise INSERT yap, değilse rollback + hata döndür
--   COMMIT;
--
-- Trigger, uygulama bu disiplini uygulamasa bile son bir güvenlik ağı
-- görevi görür (ör. doğrudan SQL ile veya farklı bir servisten yapılan
-- eklemelere karşı).
-- =====================================================================
...
BEGIN
    ...
    IF conflict_count > 0 THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Çakışma: Bu doktor için seçilen saat aralığında mevcut bir randevu var.';
    END IF;
END$$
...
BEGIN
    ...
    -- Sadece zaman/doktor/durum değişiyorsa tekrar kontrol et
...
--
-- Çakışma önleme stratejisi (2 katman):
--   1) UNIQUE(doctor_id, appointment_date, start_time) -> tam aynı
--      başlangıç saatine ikinci kaydı DB seviyesinde anında reddeder.
--   2) trg_appointments_no_overlap_insert/update -> kısmen kesişen
--      aralıkları (ör. 10:00-10:30 vs 10:15-10:45) yakalar.
--   Uygulama katmanı, race condition'a karşı SELECT ... FOR UPDATE ile
--   transaction içinde kilit almalı; trigger tek başına yeterli değildir.
-- =====================================================================
